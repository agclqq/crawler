...
class XiaohongshuBrowseUseCase:
    ...
    async def _browse_keyword(
        ...
        for iteration in range(max_iterations):
            ...
            clicked, link_id = await self._browser_service.click_random_visible_link_not_opened(
                opened_links
            )
            ...
            if content_loaded:
                ...
                # 3.2 判断内容类型（图片或视频）
                content_type = await self._browser_service.check_content_type()

                if content_type == "image":
                    # 图片内容：浏览图片
                    await self._browser_service.browse_image_content()
                elif content_type == "video":
                    # 视频内容：观看视频
                    await self._browser_service.browse_video_content()
                else:
                    logger.warning(f"Unknown content type: {content_type}, skipping media browsing")

                # 3.3 浏览评论（智能浏览，根据内容类型）
                await self._browser_service.browse_comments_smart(content_type)

                # 确保内容浏览时长至少5~20秒
                elapsed_time = asyncio.get_event_loop().time() - browse_start_time
                min_browse_time = random.uniform(3, 5)

                if elapsed_time < min_browse_time:
                    remaining_time = min_browse_time - elapsed_time
                    logger.info(
                        f"Content browsed for {elapsed_time:.2f}s, waiting additional {remaining_time:.2f}s to reach minimum {min_browse_time:.2f}s"
                    )
                    await asyncio.sleep(remaining_time)
                else:
                    logger.info(
                        f"Content browsed for {elapsed_time:.2f}s (exceeds minimum {min_browse_time:.2f}s)"
                    )
